...
for.body:                                         ; preds = %entry, %for.body
  %i.017 = phi i32 [ %inc, %for.body ], [ 2, %entry ]
  %sum.016 = phi i32 [ %sum.1, %for.body ], [ 3, %entry ]
  tail call void @DFGLOOP_TAG(i32 1) #3
  %cmp1 = icmp eq i32 %i.017, 2
  %add = add nsw i32 %sum.016, 2
  %sum.1.v = select i1 %cmp1, i32 10, i32 %i.017
  %sum.1 = add nsw i32 %sum.1.v, %sum.016
...
for.end:                                          ; preds = %for.body, %entry
  %sum.0.lcssa = phi i32 [ 3, %entry ], [ %sum.1, %for.body ]
