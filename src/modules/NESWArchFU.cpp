    ...
    : Module(name)
{
    ...
    addSubModule(new Multiplexer("mux_sel", 4));
    // for passthrough
    ...
    addConfig(new ConfigCell("MuxWSELECT"), {"mux_sel.select"});
    ...
    // to mux_b
    addConnection("this.in_n", "mux_sel.in0");
    addConnection("this.in_e", "mux_sel.in1");
    addConnection("this.in_s", "mux_sel.in2");
    addConnection("this.in_w", "mux_sel.in3");
    //addConnection("mux_out.out", "mux_a.in" + std::to_string(num_inputs));
    ...
    addConnection("mux_sel.out", "func.in_sel");
